...
def get_postgres_connection(use_admin_db: bool = False):
    ...
    try:
        ...
        logger.debug(
            f"Connecting to PostgreSQL: {'admin' if use_admin_db else 'app'} database"
        )
...
def create_users_table():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create users table: {str(e)}"
        )
...
def create_favorites_and_playlists_tables():
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Favorites and playlists tables created or already exist.",
        }
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create favorites/playlists tables: {str(e)}",
        )
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                if count >= 20:
                    raise HTTPException(
                        status_code=400, detail="Maximum 20 favorites allowed"
                    )
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                return {"tracks": [dict(row) for row in rows], "total": total}
    except Exception as e:
        raise HTTPException(
...
def update_playlist_name(user_id: int, playlist_id: int, new_name: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update playlist: {str(e)}"
        )
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                if count >= 20:
                    raise HTTPException(
                        status_code=400, detail="Maximum 20 tracks per playlist"
                    )
                ...
                # Get next position
                cursor.execute(
                    "SELECT COALESCE(MAX(position), 0) + 1 as next_pos FROM playlist_tracks WHERE playlist_id = %s;",
                    (playlist_id,),
                )
                ...
                cursor.execute(
                    "UPDATE playlists SET updated_at = NOW() WHERE id = %s;",
                    (playlist_id,),
                )
                conn.commit()
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add track to playlist: {str(e)}"
        )
...
def remove_track_from_playlist(user_id: int, playlist_id: int, track_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to remove track from playlist: {str(e)}"
        )
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                return {
                    "status": "success",
                    "message": f"Admin user {email} created with ID {user_id}.",
                }
    except Exception as e:
        raise HTTPException(
...
def update_user_password(email: str, hashed_password: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update user password: {str(e)}"
        )
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                return {
                    "status": "success",
                    "message": "Added embedding_512_vector column to megaset table.",
                }
    except psycopg2.errors.DuplicateColumn:
        return {
            "status": "info",
            "message": "embedding_512_vector column already exists.",
        }
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to add embedding_512_vector column: {str(e)}",
        )


def migrate_music_data_from_sqlite(
    bucket_name: str = "megaset-sqlite", object_name: str = "music_vector_database.db"
):
    """
    ...
    try:
        ...
        sqlite_cursor.execute(
            "SELECT id, filename, filepath, relative_path, album_folder, artist_folder, filesize, title, artist, album, year, tracknumber, genre, top_5_genres, created_at FROM songs"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to read from SQLite database: {str(e)}"
        )
    finally:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to write to PostgreSQL: {str(e)}"
        )
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT id, filepath, filename, title, artist, album, year, tracknumber, genre, filesize FROM megaset;"
                )
                songs_in_postgres = cursor.fetchall()
                logger.info(
                    f"Found {len(songs_in_postgres)} songs in PostgreSQL to process for embeddings and metadata."
                )

                if not songs_in_postgres:
                    return {
                        "status": "info",
                        "message": "No songs found in PostgreSQL to process.",
                    }
                ...
                for song_data in songs_in_postgres:
                    ...
                    try:
                        ...
                        local_pkl_path = os.path.join(
                            temp_dir, os.path.basename(object_name)
                        )

                        # 3. Download .pkl from MinIO
                        logger.debug(
                            f"Downloading {object_name} from {embeddings_bucket_name} to {local_pkl_path}"
                        )
                        download_object(
                            embeddings_bucket_name, object_name, local_pkl_path
                        )
                        ...
                        if embedding_512 is not None and (
                            not isinstance(embedding_512, np.ndarray)
                            or embedding_512.shape != (512,)
                        ):
                            raise ValueError(
                                f"Invalid embedding_512 format or shape for {object_name}"
                            )
                        ...
                        if (
                            not existing_tracknumber
                            and pkl_data.get("tracknumber") is not None
                        ):
                            tracknumber_value = pkl_data["tracknumber"]
                            if (
                                isinstance(tracknumber_value, str)
                                and tracknumber_value == ""
                            ):
                        ...
                        if (
                            not existing_filesize
                            and pkl_data.get("filesize") is not None
                        ):
                        ...
                        logger.error(
                            f"Failed to process song ID {song_id} (filepath: {filepath}): {str(e)}"
                        )
                        failed_count += 1
                    finally:
...
def find_similar_tracks_by_512_embedding(query_embedding: list[float], limit: int = 10):
    ...
    if not query_embedding or len(query_embedding) != 512:
        raise HTTPException(
            status_code=400,
            detail="Invalid query embedding. Must be a list of 512 floats.",
        )
                    ...
                    results.append(
                        {
                            ...
                            / (
                                1 + row["distance"]
                            ),  # More appropriate normalization for L2 distance
                        }
                    )
...
def query_megaset(limit: int = 100, offset: int = 0):
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM megaset ORDER BY artist, album, tracknumber LIMIT %s OFFSET %s;",
                    (limit, offset),
                )
                rows = cursor.fetchall()
                return {
