...
def create_visualization_table():
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "track_visualization table created or already exists.",
        }
...
def load_visualization_data_from_minio(
    bucket_name: str = "megaset-sqlite", object_name: str = "umap_coordinates_v2.json"
):
    """
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                # Get total count
                cursor.execute(
                    "SELECT COUNT(*) as count FROM track_visualization_umap;"
                )
    ...
    try:
        with get_postgres_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT COUNT(DISTINCT cluster) as count FROM track_visualization_umap WHERE cluster >= 0;"
                )
